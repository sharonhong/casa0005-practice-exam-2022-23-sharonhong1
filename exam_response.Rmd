...
# there are 5 Assumptions Underpinning Linear Regression
```{r Assumption 1}
library(units)
...
```
```{r}
symbox(~estimate_percent_insured_civilian_noninstitutionalized_population, 
       Regressiondata3, 
       na.rm=T,
       powers=seq(-3,3,by=.5))
...
LWard_nb <- Regressiondata3 %>%
  poly2nb(., queen=T)

model_queen <- lagsarlm(density ~ estimate_percent_below_poverty_level_population_for_whom_poverty_status_is_determined + estimate_percent_age_by_educational_attainment_population_18_to_24_years_bachelors_degree_or_higher, data = Regressiondata3, 
               nb2listw(LWard_nb, style="C",zero.policy = TRUE), 
               method = "eigen")

#something wrong with one or more iv                        
# slag_dv_model2_queen <- lagsarlm(Regressiondata3$density ~ estimate_percent_insured_civilian_noninstitutionalized_population + estimate_households_mean_income_dollars + estimate_households_total + estimate_percent_below_poverty_level_population_for_whom_poverty_status_is_determined + estimate_percent_age_by_educational_attainment_population_18_to_24_years_bachelors_degree_or_higher + estimate_percent_age_by_educational_attainment_population_18_to_24_years_high_school_graduate_includes_equivalency, data = Regressiondata3, 
#                nb2listw(LWard_nb, style="C",zero.policy = TRUE), 
#                method = "eigen")
...
Regressiondata4 <- cbind(Regressiondata3,coordsW2)
...
GWRbandwidth <- gwr.sel(density ~ 
     ...
     estimate_percent_age_by_educational_attainment_population_18_to_24_years_bachelors_degree_or_higher +
     estimate_percent_age_by_educational_attainment_population_18_to_24_years_high_school_graduate_includes_equivalency, 
                  data = Regressiondata4, 
                        coords=cbind(Regressiondata4$X, Regressiondata4$Y),
                  adapt=T)
...
Regressiondata_result <- Regressiondata2 %>%
  mutate(coef1 = estimate_percent_insured_civilian_noninstitutionalized_population,
     coef2 = estimate_households_mean_income_dollars,
     coef3 = estimate_households_total,
     coef4 = estimate_percent_below_poverty_level_population_for_whom_poverty_status_is_determined,
     coef5 = estimate_percent_age_by_educational_attainment_population_18_to_24_years_bachelors_degree_or_higher,
     coef6 = estimate_percent_age_by_educational_attainment_population_18_to_24_years_high_school_graduate_includes_equivalency)
             

tm_shape(Regressiondata_result) +
  tm_polygons(col = "coef1", 
              palette = "RdBu", 
              alpha = 0.5)
```
...
sigTest = abs(gwr.model$SDF$"estimate_percent_insured_civilian_noninstitutionalized_population")-2 * gwr.model$SDF$"estimate_percent_insured_civilian_noninstitutionalized_population_se"


#store significance results
Regressiondata_result <- Regressiondata_result %>%
